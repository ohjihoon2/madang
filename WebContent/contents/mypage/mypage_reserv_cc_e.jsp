			...
			if($("input:checkbox[id='out']").is(":checked")) {
				if($("input#out_pw_cfm").val()=="") {
					...
					if(pw_msg=="비밀번호 확인이 완료되었습니다.") {
						//alert("d");
						location.href="reserv_cc_process.jsp?p_code=<%= evo.getP_code() %>";
	...
	<div>
		...
		<div class="ticket_box_2">
			...
			<table border=1>
				<tr>
					<td>전시명</td>
					<td colspan=3><%= evo.getE_title() %></td>
				</tr>
				<tr>
					<td>매수</td>
					<td colspan=3><%= evo.getTe_cticket() %>매</td>
				</tr>
				<tr>
					<td>관람일</td>
				...
				<tr>
					...
					<td>환불금액</td>
					<td><%= (int)(evo.getTe_price()-evo.getTe_price()*0.1) %></td>
				</tr>
		...
		<div class="button">
			...
			<a href="mypage_reserv_details_e.jsp?te_code=<%= evo.getTe_code() %>"><button type="button" id="out_btn" class="button_b">취소</button></a>
		</div>
