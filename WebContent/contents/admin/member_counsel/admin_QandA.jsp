...
div#admin_qna section table td:nth-child(2){
	text-align:left;
}

div#admin_qna section table td:nth-child(2) a{
	padding: 0px 10px 0px 6px;
	text-decoration: none;
	color: gray;
}

div#admin_qna section table td:nth-child(2) a:hover {
	color: #282828;
}


div#admin_qna section table tr:last-child td{
	border:none;
}
...
<script>
	$(document).ready(function(){	
		...
		jQuery('#ampaginationsm').on('am.pagination.change',function(e){
	           ...
	           $(location).attr('href', "http://211.63.89.214:9090/contents/admin/member_counsel/admin_QandA.jsp?page="+e.page);         
	    });
	...
	<section>
		<table>
			...
			<%for(QandA_VO vo : list){ %>
				<tr>
					<td><%=vo.getRownum() %></td>
					<td><a href="admin_QandA_detail.jsp?qa_code=<%=vo.getQa_code()%>&page=<%=reqPage%>"><%=vo.getQa_title() %></a></td>
					<td><%=vo.getId() %></td>
					<td><%=vo.getQa_date() %></td>
