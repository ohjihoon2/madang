...
<%
	...
	ArrayList<Group_mem_VO> list = service.getMembergpListAdmin(startCount, endCount);
%>
...
div#admin_memberlist_group section table td:nth-child(2) a{
	padding: 0px 10px 0px 6px;
	text-decoration: none;
	color: gray;
}

div#admin_memberlist_group section table td:nth-child(2) a:hover {
	color: #282828;
}


div#admin_memberlist_group section table tr:last-child td{
	border:none;
}
</style>
<script>
	$(document).ready(function(){	
		...
		jQuery('#ampaginationsm').on('am.pagination.change',function(e){
	           ...
	           $(location).attr('href', "http://211.63.89.214:9090/contents/admin/member_counsel/admin_member_group.jsp?page="+e.page);         
	    });
	...
	<section>
		<table>
			...
			<%for(Group_mem_VO vo : list){ %>
				<tr>
					<td><%=vo.getRno() %></td>
					<td><a href="admin_member_detail_group.jsp?id=<%=vo.getId()%>&page=<%=reqPage%>"><%=vo.getId() %></a></td>
					<td><%=vo.getName() %></td>
