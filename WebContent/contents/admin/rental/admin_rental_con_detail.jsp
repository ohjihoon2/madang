...
<%
	...
	Rental_VO rvo = service.getRentalContents_con_Admin(rental_code); 
...
div#admin_rental_con_d section table{
	width:70%;
	border-collapse: collapse;
	margin-top:40px;
}

div#admin_rental_con_d section table th{
	background-color:rgb(195,195,195);
	border:1px solid gray;
	width:150px;
	height:40px;
}
div#admin_rental_con_d section > table td{
	width:35%;
	padding-left:10px;
	border:1px solid gray;
}
div#admin_rental_con_d section > table button#admin_rentalcon_d_cancel_btn,
div#admin_rental_con_d section > table button#admin_rentalcon_d_accept_btn{
...
div#admin_rental_con_d > article {
	width:70%;
	text-align :right;
	margin-top:30px;
}
div#admin_rental_con_d > article button{
	width:100px;
	height:35px;
	border:none;
	background-color:rgb(5,135,94);
...
$(document).ready(function(){
	...
	$('button#admin_rentalcon_d_accept_btn').click(function(){
		var ask = confirm("해당 대관신청을 승인하겠습니까?");
		if(ask==1){
			$.ajax({
				url:"admin_rental_proce.jsp?call=con_accept&rental_code=<%=rental_code%>",
	...
	$('button#admin_rentalcon_d_cancel_btn').click(function(){
		var ask = confirm("해당 대관신청을 취소하겠습니까?");
		if(ask==1){
			$.ajax({
				url:"admin_rental_proce.jsp?call=con_cancel&rental_code=<%=rental_code%>",
				success:function(result){
					alert(result);
					location.reload();
				}//success
...
<div id="admin_rental_con_d" class="admin_content">
	<h1>공 연 관 리</h1>	
	<section>
		<table>
			<tr>
				...
				<tr>
					...
					<td>
						...
						<%if(rvo.getR_status().equals("신청중")){ %>
							<button type="button" id="admin_rentalcon_d_accept_btn">신청완료</button>
						<%}else if(rvo.getR_status().equals("취소신청")){ %>
							<button type="button" id="admin_rentalcon_d_cancel_btn">취소완료</button>
						<%} %>
					</td>
				</tr>
				<tr>
				...
				<tr>
					<th>리허설시간</th>
					<td colspan="3"><%=rvo.getR_rhtime()%></td>
				</tr>
			</table>
			<table>
				<tr>
	...
	<article>
			<% if(rpage==null || rpage.equals(null)){ %>
				<a href="admin_rental_con.jsp"><button type="button">목록으로</button></a>
			<%}else{ %>
				<a href="admin_rental_con.jsp?page=<%=rpage%>"><button type="button">목록으로</button></a>
			<%} %>
	</article>
</body>
</html>